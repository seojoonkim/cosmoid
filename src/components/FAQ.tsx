...
const faqs = [
  {
    q: "카카오톡 대화 내용이 저장되나요?",
    a: "아니요. 클로로이드는 명령을 처리하는 데 필요한 최소한의 정보만 일시적으로 사용하고, 처리가 완료되면 즉시 폐기해요. 대화 내용을 서버에 저장하거나 학습 데이터로 활용하지 않아요. 모든 통신은 TLS 암호화로 보호되며, 제3자에게 절대 공유하지 않아요."
  },
  {
    q: "어떤 서비스들을 지원하나요?",
  ...
  {
    q: "결제 정보는 안전한가요?",
    a: "클로로이드는 카드 번호나 계좌 정보를 직접 저장하지 않아요. 모든 결제는 카카오페이, 네이버페이, 토스페이 등 공인된 PG사를 통해서만 이루어져요. 클로로이드는 '주문 요청'을 전달하는 역할만 하며, 실제 결제 승인은 사용자가 직접 확인하는 구조예요."
  },
  {
  ...
  },
  {
    ...
    a: "전혀 필요 없어요. 카카오톡에서 '클로로이드'를 친구 추가하는 것만으로 바로 시작할 수 있어요. 회원가입 폼도, 앱 다운로드도, 복잡한 설정도 없어요. 처음 대화를 시작하면 30초 안에 첫 번째 기능을 써볼 수 있어요."
  },
  {
